  ...
  <div class="card">
    ...
    <table class="table table-bordered">
      ...
      <?php 
        ...
        $sql_k = "SELECT `blog`.`id_blog`, `kategori_blog`.`kategori_blog`,
                  `blog`.`judul`, `blog`.`tanggal`
                  FROM `blog` 
        ...
        if (!empty($katakunci_blog)) {
            $sql_k .= " WHERE `kategori_blog`.`kategori_blog` LIKE '%$katakunci_blog%' OR 
                      `blog`.`judul` LIKE '%$katakunci_blog%'";
        } 

        $sql_k .= " ORDER BY `kategori_blog`.`kategori_blog`, `blog`.`judul` LIMIT $posisi, $batas";
      ...
      ?>
        <tr>
          ...
          <td align="center">
            ...
            <a href="javascript:if(confirm('Anda yakin ingin menghapus data Blog <?= $judul; ?>?'))window.location.href = 'index.php?include=blog&aksi=hapus&data=<?= $id_blog; ?>&notif=hapusberhasil'" class="btn btn-xs btn-warning"><i class="fas fa-trash" title="Hapus"></i></a>                         
          </td>
        </tr>
        <?php $no++; } ?>
      </tbody>
    ...
    <?php
      // hitung jumlah semua data blog
      $sql_jum = "SELECT `blog`.`id_blog`, `kategori_blog`.`kategori_blog`,
                `blog`.`judul`, `blog`.`tanggal`
                FROM `blog` 
                INNER JOIN `kategori_blog` ON `blog`.`id_kategori_blog` = `kategori_blog`.`id_kategori_blog`";
      ...
      // jika kata kunci tidak kosong
      if (!empty($katakunci_blog)) {
      $sql_jum .= " WHERE `kategori_blog`.`kategori_blog` LIKE '%$katakunci_blog%' OR 
                `blog`.`judul` LIKE '%$katakunci_blog%'";
      } 

      $sql_jum .= " ORDER BY `kategori_blog`.`kategori_blog`, `blog`.`judul`";
